...
interface WorkerSlot {
  ...
  failedGeneration: TimelineHistoryWorkerGeneration | null;
}
...
export const DEFAULT_TIMELINE_HISTORY_MAX_CONSECUTIVE_FAILURES = 3;
...
export class TimelineHistoryWorkerPool {
  ...
  constructor(options: TimelineHistoryWorkerPoolOptions = {}) {
    ...
    this.maxConsecutiveFailures = Math.max(1, Math.trunc(options.maxConsecutiveFailures ?? DEFAULT_TIMELINE_HISTORY_MAX_CONSECUTIVE_FAILURES));
  ...
  private handleWorkerFailure(slot: WorkerSlot, generation: TimelineHistoryWorkerGeneration, error: Error): void {
    ...
    if (slot.failedGeneration === generation) return;
    slot.failedGeneration = generation;
    slot.consecutiveFailures += 1;
    logger.warn({
      errorKind: describeError(error),
      slotId: slot.slotId,
      generation,
      consecutiveFailures: slot.consecutiveFailures,
      maxConsecutiveFailures: this.maxConsecutiveFailures,
    }, 'TimelineHistoryWorkerPool: worker failed');
    const active = slot.currentJob;
    if (active) {
      this.clearActiveTimer(active);
  ...
  private scheduleRestart(slot: WorkerSlot): void {
    ...
    if (slot.consecutiveFailures >= this.maxConsecutiveFailures) {
      ...
      logger.warn({
        ...
        consecutiveFailures: slot.consecutiveFailures,
      }, 'TimelineHistoryWorkerPool: disabling worker slot after repeated failures');
      this.drainQueueIfNoLiveCapacity();
      return;
    }
  ...
  private drainQueueIfNoLiveCapacity(): void {
    if (this.queue.length === 0) return;
    const hasLiveCapacity = this.slots.some((slot) => (
      slot.state === 'idle'
      || slot.state === 'busy'
      || (slot.state === 'restarting' && slot.restartTimer)
    ));
