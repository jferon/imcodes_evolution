...
function renderDesignRoundtablePrompt(run: EvolutionRun): string {
  return [
    ...
    '- 视觉：高保真方向、token、组件层级、动效；拒绝把骨架屏、灰色占位条或纯线框当作高保真。',
    '- 前端：是否可实现、响应式/可访问性风险；重点检查长菜单溢出、390px 文字重叠和固定栏遮挡。',
    '- 产品：设计是否仍满足 PRD 验收标准。',
    '',
    '硬性复核项：核心界面必须有真实业务语义 mock 数据、可辨识的内联 SVG 图标，并在适用场景提供图片或头像；缺任一项必须 REWORK。',
...
function renderVisualFidelityRoundtablePrompt(run: EvolutionRun): string {
  ...
  return [
    ...
    '- 内容真实性：列表/表格必须展示至少 3 行真实业务语义的 mock 数据，不得用灰色骨架条代替内容。',
    '- 图形完整性：导航、按钮、状态等需要图标的界面必须包含可辨识的内联 SVG 图标；适用的账号、商品或内容界面必须有图片或头像。',
    '- 响应式：逐屏检查长菜单是否溢出，以及 390px 视口中标题、表头、按钮和固定栏是否发生文字重叠或遮挡。',
    '',
...
function renderDesignMakerPrompt(run: EvolutionRun): string {
  ...
  return [
    ...
    '   表格/列表 props 必须写出 columns、至少 3 行真实业务语义的 mock 数据（mockRows）以及状态/操作；筛选区写出 fields；需要图形的位置写出 icon/image 或 avatar 语义。',
    `2. \`${runDirRelative}/${UI_PREVIEW_HTML_RELATIVE_PATH}\` — 自包含单文件 HTML+CSS 高保真预览：`,
    '   - 不引用任何外部资源（无外链 CSS/JS/字体/图片）；导航、搜索、按钮、状态等使用可辨识的内联 SVG 图标，不得用空方块或 emoji 冒充图标。',
    '   - 每个 screen 一个 `<section id="screen-N">`，与 ui-spec 的 screens 一一对应。',
    '   - 布局、配色、字体、间距必须与参考图可追溯对应；不得输出通用模板。',
    '   - 每个核心列表/表格至少展示 3 行真实业务语义的 mock 数据；账号/商品/内容等适用场景必须展示内联 data URI 图片或头像，不得以灰色骨架条替代。',
    '   - PC 端菜单必须使用短标签或足够宽度，禁止截断后仍表达不清；390px 预览必须逐屏自查，不得出现文字重叠、表头互压、按钮越界或桌面侧栏挤压内容。',
