...
import { EVOLUTION_PIPELINE_MSG } from '../../shared/evolution-pipeline-constants.js';
import {
...
import { handleEvolutionPipelineCommand } from './evolution-orchestrator.js';
...
function dispatchWebCommand(cmd: Record<string, unknown>, serverLink: ServerLink): void {
  switch (cmd.type) {
    ...
    case EVOLUTION_PIPELINE_MSG.USER_MESSAGE: {
      const request = cmd.request && typeof cmd.request === 'object' ? cmd.request as Record<string, unknown> : undefined;
      const evolutionSessionName = typeof cmd.sessionName === 'string'
        ? cmd.sessionName
        : (typeof request?.sessionName === 'string' ? request.sessionName : undefined);
      const sessionProjectRoot = evolutionSessionName ? getSession(evolutionSessionName)?.projectDir : undefined;
      const evolutionCmd = sessionProjectRoot && typeof cmd.projectRoot !== 'string'
        ? { ...cmd, projectRoot: sessionProjectRoot }
        : cmd;
      void traceCommandAsync(cmd, 'web_command.evolution_pipeline', () => handleEvolutionPipelineCommand(evolutionCmd, serverLink));
      break;
    }
...
function buildTimelineHistory(params: TimelineHistoryRequestParams): Promise<TimelineHistoryBuildResult> {
  ...
  if (shouldUseTimelineHistoryWorkerPool() && initialRecord?.agentType !== 'opencode') {
    return buildTimelineHistoryWithWorker(params).catch(async (err) => {
      ...
      if (
        reason === TIMELINE_HISTORY_ERROR_REASONS.PROJECTION_UNAVAILABLE
