...
describe('command-handler timeline history with SQLite-preferred reads', () => {
  ...
  it('falls back to the main SQLite projection when the history worker times out', async () => {
    shouldUseHistoryWorkerMock.mockReturnValue(true);
    ...
    readByTypesPreferredMock
      .mockResolvedValueOnce([
        { eventId: 'u-timeout', sessionId: 'deck_worker_timeout', ts: 1010, seq: 1, epoch: 1, source: 'daemon', confidence: 'high', type: 'user.message', payload: { text: 'fallback ok' } },
      ])
      .mockResolvedValueOnce([]);
    ...
    expect(readByTypesPreferredMock).toHaveBeenCalledWith(
      'deck_worker_timeout',
      expect.arrayContaining(['user.message', 'assistant.text', 'tool.result']),
      { limit: 6, afterTs: undefined, beforeTs: undefined },
    );
    expect(serverLink.send).toHaveBeenCalledWith(expect.objectContaining({
      type: TIMELINE_MESSAGES.HISTORY,
      sessionName: 'deck_worker_timeout',
      ...
      status: TIMELINE_RESPONSE_STATUS.OK,
      source: TIMELINE_RESPONSE_SOURCES.MAIN_SQLITE,
      events: [expect.objectContaining({ eventId: 'u-timeout' })],
      hasMore: false,
    }));
  });
