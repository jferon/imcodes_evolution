...
import { EvolutionWarRoomPanel } from './EvolutionWarRoom.js';
...
export function SessionControls({ ws, activeSession, inputRef, onAfterAction, onStopProject, onRenameSession, onSettings, onShareSession, sessionPinned = false, stopBlockedByPinned = false, onToggleSessionPin, subSessionId, sessionDisplayName, quickData, detectedModel, hideShortcuts, onSend, onSubRestart, onSubNew, onSubStop, activeThinking = false, activeTransportTurn = false, mobileFileBrowserOpen, onMobileFileBrowserClose, sessions, subSessions, serverId, fileDropTargetRef, quotes, onRemoveQuote, pendingPrefillText, onPendingPrefillApplied, compact, keyboardActive, onQuickOpenChange, onOverlayOpenChange, onTransportConfigSaved, onVersionSensitiveAction, onComposerTextChange }: Props) {
  ...
  const [evolutionWarRoomOpen, setEvolutionWarRoomOpen] = useState(false);
  ...
  const evolutionPipeline = useEvolutionPipeline({
    ws,
    serverId,
    sessionName: activeSession?.name,
    projectRoot: activeSession?.projectDir,
  });
  const evolutionActive = isEvolutionActiveProjection(evolutionPipeline.projection);
  ...
  useEffect(() => {
    ...
    const previousSignature = lastDaemonP2pSyncRef.current;
    // Mark the signature as in-flight before sending. React can re-run effects
    // before the no-ack save promise settles (especially in tests/StrictMode),
    // and without this optimistic guard the same loaded config may be sent
    // several times on mount.
    ...
    void persistP2pConfigToDaemon(rootSession, p2pSavedConfig, { awaitAck: false }).then((result) => {
      if (!result.ok && lastDaemonP2pSyncRef.current === signature) {
        lastDaemonP2pSyncRef.current = previousSignature;
      }
    });
  ...
  return (
    ...
    <div ref={controlsWrapperRef} class={`controls-wrapper${showRunningSweep ? ' controls-wrapper-running' : ''}${mobileComposerExpanded ? ' controls-wrapper-mobile-expanded' : ''}`}>
      ...
      {evolutionWarRoomOpen && (
        <EvolutionWarRoomPanel
          projection={evolutionPipeline.projection}
          watchers={evolutionPipeline.watchers}
          serverId={serverId}
          sessionName={activeSession?.name}
          projectRoot={activeSession?.projectDir}
          launchPending={evolutionPipeline.launchPending}
          scanPending={evolutionPipeline.scanPending}
          stagingCheckPending={evolutionPipeline.stagingCheckPending}
          skillUpdatePending={evolutionPipeline.skillUpdatePending}
          stopPending={evolutionPipeline.stopPending}
          continuePending={evolutionPipeline.continuePending}
          referenceBriefPending={evolutionPipeline.referenceBriefPending}
          lastReferenceBrief={evolutionPipeline.lastReferenceBrief}
          autoDeliverPending={openSpecAutoDeliver.launchPending}
          lastError={evolutionPipeline.lastError}
          onClose={() => setEvolutionWarRoomOpen(false)}
          onLaunch={(sourceRelativePath, options) => {
            evolutionPipeline.launch({
              sourceRelativePath,
              projectName: activeSession?.project,
              autoStartImplementation: options.autoStartImplementation,
          ...
          onSendUserMessage={(text, roleId) => { evolutionPipeline.sendUserMessage(text, roleId); }}
          onUpdateRoleSkill={(roleId, markdown) => { evolutionPipeline.updateRoleSkill(roleId, markdown); }}
          onApproveRoleSkillCandidate={(roleId, candidateArtifactId, approvalMessage, approverId) => {
            evolutionPipeline.approveRoleSkillCandidate(roleId, candidateArtifactId, approvalMessage, undefined, approverId);
          }}
          onRefresh={() => { evolutionPipeline.requestStatus(); }}
        />
      )}
      ...
      {!hideShortcuts && (!compact || showCompactMetaControls) && <div class="shortcuts-row">
        ...
        {activeSession && (
          <div class="shortcuts-model">
            <button
              class={`shortcut-btn evolution-war-room-trigger${evolutionActive ? ' evolution-war-room-trigger-active' : ''}`}
              onClick={() => setEvolutionWarRoomOpen(true)}
              disabled={disabled}
              title="Evolution War Room"
              style={{ color: evolutionActive ? '#22c55e' : '#38bdf8', fontSize: 10, fontWeight: 700 }}
            >
              Evo
            </button>
          </div>
