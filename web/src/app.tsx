...
export function App() {
  ...
  const appEvolutionTargetSession = useMemo(() => {
    const preferredName = activeSession ?? appOpenSpecAutoSessionName;
    const preferred = preferredName ? sessions.find((session) => session.name === preferredName) : null;
    return preferred
      ?? visibleMainSessions.find((session) => Boolean(session.projectDir))
      ?? visibleMainSessions[0]
      ?? null;
  ...
  const appEvolutionPipeline = useEvolutionPipeline({
    ...
    projectRoot: appEvolutionTargetSession?.projectDir ?? null,
  });
  const appEvolutionActive = isEvolutionActiveProjection(appEvolutionPipeline.projection);
  const appEvolutionDisabled = !selectedServerId;
  const appEvolutionProjectLabel = appEvolutionTargetSession?.project
    ?? appEvolutionTargetSession?.label
    ?? appEvolutionTargetSession?.name
    ?? 'No active project';
  useEffect(() => {
    if (selectedServerId) setAppEvolutionConsoleOpen(true);
  }, [selectedServerId]);
  ...
  return (
    <div class="layout" key={selectedServerId ?? ''}>
          ...
          <>
            ...
            {selectedServerId && appEvolutionConsoleOpen && (
              <EvolutionControlConsole
                ...
                sessionName={appEvolutionTargetSession?.name ?? appOpenSpecAutoSessionName}
                projectRoot={appEvolutionTargetSession?.projectDir}
                projectLabel={appEvolutionProjectLabel}
                launchPending={appEvolutionPipeline.launchPending}
                scanPending={appEvolutionPipeline.scanPending}
                lastError={appEvolutionPipeline.lastError}
                onOpenWarRoom={() => setAppEvolutionWarRoomOpen(true)}
                onLaunchDemo={() => {
                  appEvolutionPipeline.launchDemo({
                ...
                onStartDiscussion={() => setShowDiscussionDialog(true)}
                onViewDiscussions={() => {
                  setDiscussionInitialId(null);
                  setDiscussionInitialTab('team');
                  setShowDiscussionsPage(true);
                }}
                onClose={() => setAppEvolutionConsoleOpen(false)}
                mainSessionCount={visibleMainSessions.length}
                robotSessionCount={visibleSubSessions.length}
                runningDiscussionCount={discussions.filter(isBarActiveDiscussion).length}
              />
            )}
            ...
            {!resolvedActiveSessionExists && sessionsLoaded && (
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#475569', flexDirection: 'column', gap: 12 }}>
                ...
                <button class="btn btn-secondary" onClick={() => setAppEvolutionWarRoomOpen(true)}>
                  打开 Evolution Factory
                </button>
              </div>
            )}
      ...
      {appEvolutionWarRoomOpen && (
        <EvolutionWarRoomPanel
          projection={appEvolutionPipeline.projection}
          watchers={appEvolutionPipeline.watchers}
          serverId={selectedServerId ?? undefined}
          sessionName={appEvolutionTargetSession?.name ?? appOpenSpecAutoSessionName ?? undefined}
          projectRoot={appEvolutionTargetSession?.projectDir}
          launchPending={appEvolutionPipeline.launchPending}
          scanPending={appEvolutionPipeline.scanPending}
          stagingCheckPending={appEvolutionPipeline.stagingCheckPending}
          skillUpdatePending={appEvolutionPipeline.skillUpdatePending}
          stopPending={appEvolutionPipeline.stopPending}
          continuePending={appEvolutionPipeline.continuePending}
          referenceBriefPending={appEvolutionPipeline.referenceBriefPending}
          lastReferenceBrief={appEvolutionPipeline.lastReferenceBrief}
          autoDeliverPending={appOpenSpecAutoDeliver.launchPending}
          lastError={appEvolutionPipeline.lastError}
          onClose={() => setAppEvolutionWarRoomOpen(false)}
          onLaunch={(sourceRelativePath, options) => {
            appEvolutionPipeline.launch({
              sourceRelativePath,
              projectName: appEvolutionTargetSession?.project,
          ...
          onSendUserMessage={(text, roleId) => { appEvolutionPipeline.sendUserMessage(text, roleId); }}
          onUpdateRoleSkill={(roleId, markdown) => { appEvolutionPipeline.updateRoleSkill(roleId, markdown); }}
      ...
      {showNewSession && (
        <NewSessionDialog
          ...
          onSessionStarted={(name) => {
            ...
            setAppEvolutionConsoleOpen(false);
          }}
