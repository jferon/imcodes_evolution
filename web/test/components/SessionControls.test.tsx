...
function gatherP2pConfigSaves(ws: ReturnType<typeof makeWs>): Array<Record<string, unknown>> {
  return ws.send.mock.calls
    .map(([message]) => message as Record<string, unknown>)
    .filter((message) => message?.type === P2P_CONFIG_MSG.SAVE);
}
...
afterEach(() => {
  ...
  it('re-sends the current P2P config when the daemon reconnects', async () => {
    ...
    await flushAsync();
    await waitFor(() => expect(gatherP2pConfigSaves(ws).length).toBeGreaterThanOrEqual(1));
    const initialSaveCount = gatherP2pConfigSaves(ws).length;

    ws.emit({ type: DAEMON_MSG.RECONNECTED });

    await waitFor(() => expect(gatherP2pConfigSaves(ws)).toHaveLength(initialSaveCount + 1));
    expect(gatherP2pConfigSaves(ws).at(-1)).toEqual({
      type: P2P_CONFIG_MSG.SAVE,
      requestId: expect.any(String),
      scopeSession: 'my-session',
      config: {
